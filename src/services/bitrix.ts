...
function buildFormData(payload: CreateLeadPayload): URLSearchParams {
    ...
    // optional: register in live feed
    fd.set('params[REGISTER_SONET_EVENT]', 'Y');
    return fd;
}

/**
 * Creates a Bitrix24 lead using Inbound Webhook URL.
 * Define VITE_BITRIX_WEBHOOK_URL in your environment, e.g.
 * https://your-portal.bitrix24.ru/rest/<user_id>/<webhook_token>/crm.lead.add.json
 */
export async function createLead(payload: CreateLeadPayload): Promise<{ id: number } | null> {
    const webhookUrl = import.meta.env.VITE_BITRIX_WEBHOOK_URL as string | undefined;
    if (!webhookUrl) {
        console.warn('VITE_BITRIX_WEBHOOK_URL is not set. Lead will not be created.');
        return null;
    }
